...
| Fill reconciliation from tradebook | 🚧 | Added on-demand `/trades/reconcile` + minute-level grouped orderbook snapshot reconciliation + tradebook snapshot enrichment (`/trades`, `/orders/:id/trades`) + partial-fill rollup metadata persistence; true streaming push sync pending |
...
| Portfolio optimization baseline | ✅ | `POST /backtesting/optimize-portfolio` now supports min/max weights, max-active constraints, candidate-count controls, and ranks feasible allocations |

## Advanced Platform

| Capability | Status | Notes |
|---|---|---|
| Multi-provider AI orchestration | 🚧 | Deterministic provider registry + optional live OpenAI/Anthropic inference with retries/timeout/fallback + persisted decision logs + governance summary/ledger APIs + profile-managed daily budget policy guardrails + admin approval queue baseline for policy updates; multi-step org approver hierarchy still pending |
| AI cost ledger baseline | ✅ | Daily rollup table + scheduler + governance ledger API shipped |
| AI governance event trail | ✅ | Live policy allow/block events persisted + API + dashboard rendering |
